...
int isMandelbrot(double&, double&, int&);
int isJulia(double&, double&, int&);
...
void AnimateSet() {
	impulse--;
	std::thread *Animator2 = new std::thread(ResetRenderTarget, 0, Width / 4, 0, Height);
	std::thread *Animator3 = new std::thread(ResetRenderTarget, Width / 4, Width / 2, 0, Height);
	std::thread *Animator4 = new std::thread(ResetRenderTarget, Width / 2, Width * 3/4, 0, Height);
...
void RenderInTarget(int x_0, int x_1, int y_0, int y_1) {
	...
	for (int x = x_0; x < x_1; x++) {
		for (int y = y_0; y < y_1; y++) {
			Transform(x, y, &Re, &Im);
			PointDepth[x][y] = inSet(Re, Im, quality);
			SetColorBuffer(x, y, PointDepth[x][y]);
		}
	}
}
...
void Render() {
	...
	std::thread *Render2 = new std::thread(RenderInTarget, 0, Width/2, 0, Height/2);
	std::thread *Render3 = new std::thread(RenderInTarget, 0, Width/2, Height/2, Height);
	std::thread *Render4 = new std::thread(RenderInTarget, Width/2, Width, 0, Height/2);
		...
		case 2: 
			{
				...
				if (dn > 50) dn = 128 - dn;
				if (dn < 50) {
					/*RenderTarget[x][y].r = n*n / (float)(quality*quality) + Impulses[(n - (impulse & 127) >> 7) % 210][dn].r;
					RenderTarget[x][y].g = n*n / (float)(quality*quality) + Impulses[(n - (impulse & 127) >> 7) % 210][dn].g;
					RenderTarget[x][y].b = n / (float)quality + Impulses[(n - (impulse & 127) >> 7) % 210][dn].b;*/
					RenderTarget[x][y].r = n*n / (float)(quality*quality) + Impulses[(impulse - n) % 210][dn].r;
					RenderTarget[x][y].g = n*n / (float)(quality*quality) + Impulses[(impulse - n) % 210][dn].g;
					RenderTarget[x][y].b = n / (float)quality + Impulses[(impulse - n) % 210][dn].b;
					return;
				}
			}
			break;
...
void InitColorBuffers() {
	...
	for (int i = 0; i < 50; i++) {
		Liquid[i] = Gradient3(cyan, black, i, 50);
	}
	for (int i = 0; i < 210; i++) {
		for (int j = 0; j < 50; j++) {
